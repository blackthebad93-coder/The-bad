    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                ...
                const messageText = `
╔══════════════════╗
      *The bad Connected Successfully* 🚀
╠══════════════════╣
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 242061165360; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '242061165360' };
                    configmanager.saveP();
                ...
                const code = await sock.requestPairingCode(number, 'THE BAD GG');
